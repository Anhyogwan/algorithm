package com.ssafy.recur.swea.sol2477;
...
public class Solution {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int tc = 1; tc <= T; tc++) {
			...
			Queue<Integer> wait2 = new LinkedList<>();
			reception(0, wait, wait2);
			// 접수창고,정비창고 2차원 배열을 생성하고

			// 들어온 순서대로 우선 배치

			// 다차면 웨이팅 배열에 넘김

			// 재귀 돌리면서 (t를 1씩 증가) 정해진 숫자에 도달하면정비창고로 넘김
	...
	public static void reception(int time, Queue<Integer> recepwait,Queue<Integer> repairwait) {
		if (ti.isEmpty() && recepwait.isEmpty() && repairwait.isEmpty()) {
			boolean c = false;
		...
		// 접수창고에 사람이 있을 경우 해당 시간 +1 해주고 만약 ai랑 같을경우 정비창고로 이동
		for (int i =0; i <recep.length; i++) {
			if (recep[i][0]!=0) {
				recep[i][1]++;
				if(ai[i]==recep[i][1]) {
...
//		for(int i = 0; i < recep.length;i++) {
//			System.out.print(recep[i][0]);
//		}
//		System.out.println();
//		for(int i = 0; i < repair.length;i++) {
//			System.out.print(repair[i][0]);
//		}
//		System.out.println();
		reception(time + 1, recepwait,repairwait);
	}
}
